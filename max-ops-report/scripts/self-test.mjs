...
const dryStart = await run(["start", ...adapterContext, "--task", "rec-demo", "--title", "Started", "--detail", "Scoped task read", "--key", "future-agent:run-001:start:001", "--dry-run"]);
...
try {
  ...
  const connected = await run(["connect", "--url", "https://mock.maxops.test", "--task", "rec-1", "--agent-id", "future-agent", "--agent-name", "Future Agent", "--run", "future-agent:run-001"]);
  assert.equal(connected.ok, true);
  assert.equal(connected.adapter, "codex-reference");
  assert.equal(connected.session.task_id, "rec-1");
